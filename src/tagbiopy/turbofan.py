...
def set_classifier(classifier_name='logit', **kwargs):
    ...
    if classifier_name == 'logit':
        ...
        ret = MLPClassifier(ssolver='adam', random_state=42)
...
def train_n_predict(_df, fail_within, m, tau, classifier_name, **kwargs):
    logger.debug('Start train_n_predict')
    ...
    # Ground truth for both training and test data
    ...
    q_self.name = f'Train: {outcome_column}, m={m}, tau={tau}, {classifier_name} classifier'
    logger.info(f'\n{q_self}')

    q = quality_of_fit(y[TEST], y_hat[TEST])
    q.name = f'Test: {outcome_column}, m={m}, tau={tau}, {classifier_name} classifier'
    logger.info(f'\n{q}')
...
class Preparer:
    ...
    def binary_outcome(self):
        if self._binary_outcome is None:
            logger.debug(f'df columns: {self.df.columns}')
