...
class RunNotebook(SDKInput):
    ...
    def modified_notebook_file_input(self):
        if self._modified_notebook_file_input is None:
            ...
            self._modified_notebook_file_input = create_temp_file(
                prefix=f"{self._prefix}-{now(sep='_')}", suffix=self._suffix
            )
            ...
            with open(self._modified_notebook_file_input, "w") as fh:
                json.dump(self._modify_notebook_content(), fh, indent=2)
    ...
    def modified_notebook_file_executed(self):
        ...
        self._modified_notebook_file_executed = (
            self.modified_notebook_file_input.replace(prefix, f"{prefix}_executed")
        )
    ...
    def _modify_notebook_content(self):
        ...
        logger.debug(
            f"{self}: Modified original notebook content by adding the first cell."
        )
        return s
    ...
    def execute_notebook(self):
        ...
        logger.debug(
            f"{self}: Create report from {self.modified_notebook_file_executed}"
        )
...
class RunFunction(SDKInput):
    ...
    def __call__(self):
        self.user_function(tag_data=self.tag_data, tag_result=self.tag_result)
    ...
    def tag_data(self):
        if self._tag_data is None:
            ...
            logger.info("Check TagbioData.df ... ")
    ...
    def tag_result(self):
        if self._tag_result is None:
            ...
            self._tag_result = TagbioResult(
                extension=self.output_type, path=self.output_file
            )
...
class Run:
    def __init__(self, args):
        ...
        if args.user_function.endswith(".py"):
            ...
            logger.info(
                message.format(what="jupyter notebook", where=args.user_function)
            )
            self._class = RunNotebook(args)
        else:
            msg = (
                f"Invalid input: {args.user_function!r}. Provide either a .py or .ipynb"
            )
...
class FCPacket:
    def __init__(self, filename, uid=None):
        ...
        self.passthrough_arguments = PassThroughArguments(
            self._packet.get("passthrough_arguments")
        )
    ...
    def _log_script(self):
        ...
        if self.analysis_variables is not None:
            logger.debug(
                f"{self!r}: analysis variables: {json.dumps(self.analysis_variables, indent=4)}"
            )
        else:
    ...
    def save(self, what="fig", **kwargs):
        if what == "fig":
            ...
            if isinstance(self.fig, plotly.graph_objects.Figure):
                ...
                self.fig.savefig(
                    self.path, format=self.extension, bbox_inches="tight", **kwargs
                )
            else:
            ...
            if self.extension != "csv":
                message = 'Your output extension is {}. Setting to "csv"'.format(
                    self.extension
                )
